...
# --- 1) LISTA DE PROYECTOS (puedes pegar tu lista aquÃ­ o dejar vacÃ­o para autodetectar) ---
$projects = @"
...
# --- 2) Si $projects estÃ¡ vacÃ­o, autodetecta proyectos con .idea ---
if (-not $projects -or $projects.Count -eq 0) {
  Write-Host "ðŸ”Ž Autodetectando proyectos en C:\ ..." -ForegroundColor Cyan
...
foreach ($proj in $projects) {
  if (Test-Path $proj) {
    ...
    # Copiar con robocopy (mÃ¡s robusto que Copy-Item)
    $xd = @()
    ...
    if (-not (Test-Path $reqFile)) {
      try {
        Write-Host "ðŸ“ Generando requirements.txt en $proj ..."
        # Detectar dependencias importadas en los .py (muy bÃ¡sico)
        ...
        if ($imports) {
          ...
          "### No se detectaron imports automÃ¡ticamente" | Out-File -FilePath $reqFile -Encoding UTF8
        }
      } catch {
        Write-Host "âš ï¸ Error generando requirements.txt en $proj"
      }
    }
    $copied++
  } else {
...
Write-Host "`nâœ… Respaldo COMPLETO: carpeta + ZIP + requirements.txt listos en D:\" -ForegroundColor Green
