...
st.title("ðŸ›¡ï¸ AmdaOps - Visualizador de frases operativas")
...
selected_prefix = st.sidebar.selectbox("ðŸ·ï¸ Seleccionar sitio por prefijo", prefixes)

# Obtener datos del sitio
site_info = get_site_by_prefix(registry, selected_prefix)
if site_info:
    st.subheader("ðŸ“ InformaciÃ³n del sitio seleccionado")
    st.markdown(f"""
...
category = st.selectbox("ðŸ“‚ CategorÃ­a", [""] + categories)

hotwords = get_hotwords(filtered_phrases)
hotword = st.selectbox("ðŸ” Palabra clave (hotword)", [""] + hotwords)
custom_hotword = st.text_input("âœï¸ O escribe una palabra clave personalizada")
final_hotword = custom_hotword if custom_hotword else hotword

limit = st.slider("ðŸ”¢ Cantidad de frases", 1, 20, 5)

# Buscar frases
if st.button("ðŸ”Ž Buscar frases"):
    resultados = []
    ...
    st.subheader("ðŸ§  Frases encontradas")
    if resultados:
        for p in resultados:
...
st.markdown("---")
st.subheader("ðŸ“‹ Todas las frases en esta hoja")
